...
export const CreateContentPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadDraft = async () => {
      ...
      try {
        const draft = await supabaseService.getDraft(user.id);
        if (draft && draft.draft_data) {
          setFormData(draft.draft_data);
          setLastSaved(new Date(draft.autosaved_at));
        }
      } catch (err) {
        console.error('Error al cargar draft:', err);
      }
    };
  ...
  useEffect(() => {
    ...
    const saveDraft = async () => {
      if (formData.title || formData.description || formData.contentHtml) {
        setIsSaving(true);
        try {
          await supabaseService.saveDraft(user.id, formData);
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    try {
      ...
      const publicUrl = await supabaseService.uploadFile(file, path);
  ...
  const validateForm = (): string | null => {
    ...
    if (formData.contentType === 'TEXT' && !formData.contentHtml.trim()) return 'El contenido es requerido para tipo TEXT';
  ...
  const handleSubmit = async (asScheduled = false, scheduledFor?: Date) => {
    setError(null);
    ...
    try {
      ...
      const contentData = {
        ...
        price: formData.isPremium && !formData.isFree ? formData.price : 0,
      ...
      const content = await supabaseService.createContent(contentData);

      if (asScheduled && scheduledFor && content) {
        await supabaseService.schedulePost(content.id, scheduledFor.toISOString(), user.id);
      }

      // Eliminar draft después de publicar
      const draft = await supabaseService.getDraft(user.id);
      if (draft) {
  ...
  const handleSchedule = (scheduledFor: Date) => {
    ...
    handleSubmit(true, scheduledFor);
  };
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Rich Text Editor para tipo TEXT */}
